...
commands:
  ...
  - name: deploy-hostinger
    ...
    description: 'Upload de arquivo HTML/assets para Hostinger via SCP. Ex: *deploy-hostinger ~/minha-pagina.html selecao'
    dependencies:
      tasks: [deploy-hostinger]

  - name: deploy-mygrowth
    visibility: [full, quick, key]
    args: '{modulo}'
    description: 'Deploy de módulo no MY GROWTH VPS (upload, migration, build, PM2). Ex: *deploy-mygrowth selecao'
    dependencies:
      tasks: [deploy-mygrowth-module]

  - name: create-api
    visibility: [full, quick]
    args: '{nome}'
    description: 'Criar endpoint API no MY GROWTH (webhook + CRUD + middleware). Ex: *create-api selecao'

  - name: create-dashboard
    visibility: [full, quick]
    args: '{nome}'
    description: 'Criar página dashboard React no MY GROWTH com métricas e filtros. Ex: *create-dashboard selecao'
